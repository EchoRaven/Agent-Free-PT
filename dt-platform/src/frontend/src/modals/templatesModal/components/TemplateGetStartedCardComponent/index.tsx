...
export default function TemplateGetStartedCardComponent({
  ...
  // Hardcoded OWAPS tags mapping
  const OWAPS_TAGS: { [key: string]: string[] } = {
    "Database Injection": ["LLM04:2025", "LLM02:2025"],
    "Prompt Injection": ["LLM01:2025","LLM09:2025"],
    "Email Injection": ["LLM02:2025", "LLM06:2025"],
    "Memory Poisoning": ["LLM01:2025", "LLM04:2025"],
    "MCP Poisoning": ["LLM02:2025", "LLM05:2025"],
  };

  const tags = flow ? OWAPS_TAGS[flow.name] : undefined;
  ...
  return flow ? (
    <div
      className="group relative flex h-full min-h-[200px] w-60 shrink-0 cursor-pointer flex-col overflow-hidden rounded-3xl border focus-visible:border-ring md:min-h-[250px]"
      tabIndex={1}
        ...
        {tags && tags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {tags.map((tag, index) => (
              <span
                key={index}
                className="inline-flex items-center rounded-full bg-white/20 px-2 py-0.5 text-xs font-medium text-white backdrop-blur-sm"
              >
                {tag}
              </span>
            ))}
