...
export function FlowSidebarComponent({ isLoading }: FlowSidebarComponentProps) {
  ...
  const finalFilteredData = useMemo(() => {
    ...
    // Filter out logic category
    const { logic, ...filteredDataWithoutLogic } = filteredData;
    filteredData = filteredDataWithoutLogic;

    // Filter out embedding model from models category
    if (filteredData.models) {
      filteredData.models = Object.fromEntries(
        Object.entries(filteredData.models).filter(([key, _]) => !key.toLowerCase().includes('embedding'))
      );
    }

    // Whitelist approach: Only show specific components in data category
    if (filteredData.data) {
      const allowedDataComponents = [
        'File',              // FileComponent -> name = "File"
        'SQLComponent',      // SQLComponent -> name = "SQLComponent"  
        'WebSearchNoAPI',    // WebSearchComponent -> name = "WebSearchNoAPI"
        'Email',             // EmailComponent -> name = "EmailComponent"
      ];
      filteredData.data = Object.fromEntries(
        Object.entries(filteredData.data).filter(([key, _]) =>
          allowedDataComponents.some(allowed => key.includes(allowed))
        )
      );
    }

    // Whitelist approach: Only show specific components in processing category
    if (filteredData.processing) {
      const allowedProcessingComponents = [
        'ParserComponent',        // ParserComponent -> uses class name (no explicit name field)
        'Prompt Template',        // PromptComponent -> name = "Prompt Template" 
        'StructuredOutput',       // StructuredOutputComponent -> name = "StructuredOutput"
        'TypeConverterComponent', // TypeConverterComponent -> uses class name (no explicit name field)
      ];
      filteredData.processing = Object.fromEntries(
        Object.entries(filteredData.processing).filter(([key, _]) =>
          allowedProcessingComponents.some(allowed => key.includes(allowed))
        )
      );
    }
