...
# User limits
max_users: int = 50
"""Maximum number of users that can be created. Set to -1 for unlimited."""
...
async def add_user(
    ...
    if max_users > 0:
        current_user_count = (await session.exec(select(func.count(User.id)))).one()
        print(f"Current user count: {current_user_count}, Max users: {max_users}")
        if current_user_count >= max_users:
            raise HTTPException(
                status_code=400,
                detail=f"Maximum number of users ({max_users}) reached"
            )
