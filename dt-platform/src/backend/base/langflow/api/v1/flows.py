    ...
    *,
    ...
    try:
        ...
        if not starter_folder:
            # If no starter folder exists in DB, load demos directly from JSON files
            ...
            # Load all JSON demo files
            for json_file in starter_projects_dir.glob("*.json"):
                try:
                    with open(json_file, "r", encoding="utf-8") as f:
                        project_data = json.load(f)
                        # Create a FlowRead object from the JSON data
                        flow_read = FlowRead(
                            id=project_data.get("id", str(json_file.stem)),
                            name=project_data.get("name", json_file.stem.replace("_", " ").title()),
                            description=project_data.get("description", "DecodingTrust-Agent Security Demo"),
                            data=project_data.get("data", project_data),
                            is_component=project_data.get("is_component", False),
                            ...
                            icon_bg_color=project_data.get("icon_bg_color"),
                            gradient=project_data.get("gradient"),
                            tags=project_data.get("tags", [])
                        )
                        demo_flows.append(flow_read)
                except Exception:
                    continue

            if demo_flows:
                all_starter_folder_flows_response = compress_response(demo_flows)
                return all_starter_folder_flows_response
            return []
