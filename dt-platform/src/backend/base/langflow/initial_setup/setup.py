...
def update_template_of_project_data(project_name, project_data):
    VIRTUE_GUARD_DASHBOARD_HOSTNAME = os.getenv("VIRTUE_GUARD_DASHBOARD_HOSTNAME", "https://guard-dashboard.virtueai.io")
    VIRTUEAGENT_SERVICE_URL = os.getenv("VIRTUEAGENT_SERVICE_URL", "http://virtueagent-service.agent.svc.cluster.local:8010")
    VIRTUEAGENT_GUARD_TEMPLATE_API_KEY = os.getenv("VIRTUEAGENT_GUARD_TEMPLATE_API_KEY", "VIRTUEAI_API_KEY")
    OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "OPENAI_API_KEY")

    logger.info(f"Updating template of project: {project_name}")
    ...
    for node in project_data.get("nodes", []):
        if "data" in node and "node" in node["data"] and "template" in node["data"]["node"]:
            ...
            if "base_url" in template:
                template['base_url']['value'] = VIRTUEAGENT_SERVICE_URL
            if "dashboard_url" in template:
                template['dashboard_url']['value'] = f"{VIRTUE_GUARD_DASHBOARD_HOSTNAME}/dashboard/virtueagent"
            if "api_key" in template:
                if template['api_key']['display_name'] == "OpenAI API Key":
                    template['api_key']['value'] = OPENAI_API_KEY
                elif template['api_key']['display_name'] == "API Key":
                    template['api_key']['value'] = VIRTUEAGENT_GUARD_TEMPLATE_API_KEY
                else:
                    template['api_key']['value'] = "KEY_REQUIRED"
    ...
    async with session_scope() as session:
        ...
        if get_settings_service().settings.update_starter_projects:
            ...
            for project_path, project in starter_projects:
                ...
                updated_project_data = update_template_of_project_data(project_name, updated_project_data)
